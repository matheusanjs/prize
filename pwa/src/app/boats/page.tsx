...
import {
  respondToSwap,
  confirmArrival, cancelReservation,
} from '@/services/api';
import { format, parseISO, isToday } from 'date-fns';
import {
  useSharesQuery,
  useChargesByBoatQuery,
  usePendingSwapsQuery,
  useTodayReservationsBoatsQuery,
  useHighlightedTripsQuery,
  useWeatherQuery,
} from '@/hooks/queries/useBoatsQuery';
...
export default function BoatsPage() {
  ...
  const sharesQ = useSharesQuery(userId);
  const chargesByBoatQ = useChargesByBoatQuery(userId);
  const pendingSwapsQ = usePendingSwapsQuery(userId);
  const todayResQ = useTodayReservationsBoatsQuery(userId);
  const highlightedTripsQ = useHighlightedTripsQuery(userId);
  const weatherQ = useWeatherQuery();

  const shares = useMemo(() => sharesQ.data || [], [sharesQ.data]);
  const chargesByBoat = useMemo(() => chargesByBoatQ.data || {}, [chargesByBoatQ.data]);
  const pendingSwaps = useMemo<SwapRequest[]>(() => pendingSwapsQ.data || [], [pendingSwapsQ.data]);
  const todayReservations = useMemo(() => todayResQ.data || [], [todayResQ.data]);
  const highlightedTrips = useMemo(() => highlightedTripsQ.data || [], [highlightedTripsQ.data]);
  const weather = weatherQ.data || null;

  const loading = sharesQ.isLoading && !sharesQ.data;
  const tripsLoaded = !highlightedTripsQ.isLoading;
  ...
  const handleSwapRespond = async (swapId: string, accept: boolean) => {
    ...
    try { await respondToSwap(swapId, accept); pendingSwapsQ.refetch(); }
...
const SectionLabel = memo(function SectionLabel({ icon, color, label }: { icon: React.ReactNode; color: string; label: string }) {
  return (
